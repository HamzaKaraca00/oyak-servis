...
test('driver locations remain visible only to same-service personnel and can be cleared from staff history', async () => {
  ...
  try {
    ...
    const createUser = async (name, phone, sicilNo, password, role = 'personel') => {
      const initial = await getCsrf(port);
    ...
    const sameServiceStaff = await createUser('Aynı Servis Personeli', `05${String(Date.now()).slice(-9)}`, String(Date.now()).slice(-10), 'secret123');
    const otherServiceStaff = await createUser('Başka Servis Personeli', `05${String(Date.now() + 1234).slice(-9)}`, String(Date.now() + 1234).slice(-10), 'secret123');
    const driver = await createUser('Sürücü 01', `05${String(Date.now() + 4567).slice(-9)}`, String(Date.now() + 4567).slice(-10), 'secret123', 'driver');

    assert.equal(sameServiceStaff.response.status, 201);
    assert.equal(otherServiceStaff.response.status, 201);
    assert.equal(driver.response.status, 201);

    const staffJoin = await fetch(`http://localhost:${port}/api/join-service`, {
      method: 'POST',
      headers: authHeaders(sameServiceStaff.cookie, sameServiceStaff.csrf, { 'Content-Type': 'application/json' }),
      body: JSON.stringify({ serviceId: 'service-01' })
    });
    const otherStaffJoin = await fetch(`http://localhost:${port}/api/join-service`, {
      method: 'POST',
      headers: authHeaders(otherServiceStaff.cookie, otherServiceStaff.csrf, { 'Content-Type': 'application/json' }),
      body: JSON.stringify({ serviceId: 'service-02' })
    });
    const driverJoin = await fetch(`http://localhost:${port}/api/join-service`, {
      method: 'POST',
    ...
    assert.equal(otherStaffJoin.status, 200);
    assert.equal(driverJoin.status, 200);
    ...
    const notifyResponse = await fetch(`http://localhost:${port}/api/notify`, {
      ...
      body: JSON.stringify({
        ...
        coordinates: { latitude: 40.1234, longitude: 29.5678 },
        idempotencyKey: `location-${Date.now()}`
      })
    });
    assert.equal(notifyResponse.status, 201);

    const sameServiceNotifications = await fetch(`http://localhost:${port}/api/notifications`, {
      headers: authHeaders(sameServiceStaff.cookie, sameServiceStaff.csrf)
    });
    const sameServiceData = await sameServiceNotifications.json();
    assert.equal(sameServiceNotifications.status, 200);
    assert.ok(sameServiceData.some((entry) => entry.type === 'driver_location' && entry.userId === sameServiceStaff.data.user.id));

    const otherServiceNotifications = await fetch(`http://localhost:${port}/api/notifications`, {
      headers: authHeaders(otherServiceStaff.cookie, otherServiceStaff.csrf)
    });
    const otherServiceData = await otherServiceNotifications.json();
    assert.equal(otherServiceNotifications.status, 200);
    assert.ok(!otherServiceData.some((entry) => entry.type === 'driver_location'));

    const clearResponse = await fetch(`http://localhost:${port}/api/notifications/clear`, {
      method: 'POST',
      headers: authHeaders(sameServiceStaff.cookie, sameServiceStaff.csrf)
    });
    assert.equal(clearResponse.status, 200);
    const clearedResponse = await fetch(`http://localhost:${port}/api/notifications`, {
      headers: authHeaders(sameServiceStaff.cookie, sameServiceStaff.csrf)
    });
    const clearedData = await clearedResponse.json();
    assert.equal(clearedResponse.status, 200);
    assert.equal(clearedData.length, 0);
  } finally {
    child.kill('SIGTERM');
    await new Promise((resolve) => setTimeout(resolve, 300));
  }
